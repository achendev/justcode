 ...
 * @param {string} [delimiter='EOPROJECTFILE'] The heredoc delimiter used in the content string.
 * @returns {string[]} An array of content chunks.
 */
export function splitContextPayload(contentString, splitSizeKb, delimiter = 'EOPROJECTFILE') {
    ...
    // Create dynamic regex based on the delimiter
    const regex = new RegExp(`cat > .*? << '${delimiter}'[\\s\\S]*?${delimiter}\\n\\n`, 'g');
    const fileBlocks = contentString.match(regex) || [];
