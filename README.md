...
6.  **Add Your Task:** This is the key step. Press Press space or start typing to scroll to the **bottom** of the large code block that was just pasted, and add your instructions for the LLM. For example: "Add a new endpoint called `/status` that returns `{'status': 'ok'}`" or "Refactor the `handle_user_login` function to be more secure".
...
## 📂 Project Structure
    ...
    │   ├── rollback_endpoint.py    # Handles /rollback endpoint
    ...
    │   │   ├── rollback.js         # Logic for reverting a deployment
